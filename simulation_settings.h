...
// JART VCM v1b var
const double memristor_bisection_a = 0.5;            // Dampening factor of the bisection solver
...
const double memristor_dynamic_time_step_N_limit = 1e-1;   // Changes in N above this limit trigers the dynamic time step mechanism
const double memristor_dynamic_time_step_t_limit = 1e-9;   // Delta times below this value will prevent a time step from being triggered
const int memristor_dynamic_time_step_time_division = 10;  // The amount of subdivisions of the original time step that trigered the dynamic time step mechanism
...
const float non_linear_fixed_point_voltage_nudge = 1e-6;  // Amount the voltage guess will be nudged by if a NAN is returned from the linear solver
